...
(function () {
  ...
  // ---- Matches used for this page: live-fetched (if sport.liveSource is
  //      set in config.js and the fetch has resolved) or the manual
  //      "matches" array in config.js as a fallback ----
  var liveMatches = null; // filled in by refreshLiveData() below, once

  function currentMatches() {
    return (liveMatches && liveMatches.length) ? liveMatches : sport.matches;
  }
  ...
  // ---- If this sport has a liveSource (see config.js + live-data.js),
  //      fetch real fixtures and re-render once they arrive. Silently
  //      keeps the config.js fallback if the fetch fails. ----
  if (Live && sport.liveSource) {
    Live.fetchMatches(sport.liveSource).then(function (matches) {
