...
(function () {
  ...
  var Live = window.LiveData;

  // ---- Cache of live-fetched matches per sport key, filled in
  //      asynchronously by refreshLiveSports() below. Until a sport's
  //      live fetch succeeds (or if it has no liveSource at all), its
  //      manual config.js "matches" array is used as a fallback — so the
  //      page always has something to show immediately, never a blank
  //      screen while waiting on the network. ----
  var liveMatchesCache = {};

  function matchesForSport(sport) {
    var live = liveMatchesCache[sport.key];
  ...
  function buildAllMatches() {
    ...
    Object.keys(cfg.sports).forEach(function (key) {
      ...
      var annotated = Sched.annotate(matchesForSport(sport), sport.defaultDurationMinutes);
  ...
  // ---- Category tabs: filter by sport (All / Football / Baseball / ...),
  //      combined with whatever's typed in the search box ----
  ...
  function renderCategoryTabs() {
    ...
    Object.keys(cfg.sports).forEach(function (key) {
      ...
      btn.type = 'button';
      btn.className = 'category-tab' + (activeCategory === key ? ' active' : '');
      btn.textContent = sport.icon + ' ' + sport.label;
  ...
  function renderAll() {
    ...
    var byCategory = activeCategory === 'all'
      ? allMatches
      : allMatches.filter(function (m) { return m.sportKey === activeCategory; });
    var filtered = query ? byCategory.filter(function (m) { return matchesQuery(m, query); }) : byCategory;
    var sortedFiltered = Sched.sortForDisplay(filtered);
    var sortedAll = Sched.sortForDisplay(allMatches);

    renderHero(sortedAll); // hero always reflects the real global state, not the search/category filter
  ...
  //      its OWN auto-computed live/next status. Re-run after live data
  //      loads so a sport's "Next:" line updates once real fixtures land. ----
  ...
  function renderSportsGrid() {
    ...
    Object.keys(cfg.sports).forEach(function (key) {
      var sport = cfg.sports[key];
      var sportFeatured = Sched.getFeatured(matchesForSport(sport), sport.defaultDurationMinutes);
  ...
  // ---- Pull real fixtures for any sport that has a "liveSource" in
  //      config.js (see live-data.js). Runs once on load, then again
  //      every few minutes — kept infrequent to stay well within the
  //      free API's rate limit. Falls back silently to config.js's
  //      manual "matches" array for a sport if the fetch fails. ----
  function refreshLiveSports() {
    if (!Live) return;
    var sportsWithLiveSource = Object.keys(cfg.sports)
      .map(function (key) { return cfg.sports[key]; })
      .filter(function (sport) { return sport.liveSource; });

    if (!sportsWithLiveSource.length) return;

    var fetches = sportsWithLiveSource.map(function (sport) {
      return Live.fetchMatches(sport.liveSource).then(function (matches) {
        if (matches && matches.length) {
          liveMatchesCache[sport.key] = matches;
        }
      });
    });
  ...
  refreshLiveSports();
  setInterval(refreshLiveSports, 5 * 60000); // every 5 minutes
})();
