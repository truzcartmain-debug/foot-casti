...
(function () {
  ...
  // ---- "Browse by Sport" cards — each links to its OWN page ----
  var sportsGrid = document.getElementById('sports-grid');
  if (sportsGrid && cfg.otherSports) {
    Object.keys(cfg.otherSports).forEach(function (key) {
      var sport = cfg.otherSports[key];
      var card = document.createElement('a');
      card.className = 'sport-card';
      card.href = sport.page;

      var teams = sport.liveMatch ? (sport.liveMatch.home + ' vs ' + sport.liveMatch.away) : '';
      ...
      card.innerHTML =
        ...
        '<div class="sport-card-body">' +
          ...
          (teams ? '<div class="sport-card-match">' + teams + '</div>' : '') +
        '</div>' +
